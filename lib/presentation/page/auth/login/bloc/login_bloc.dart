...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  */  
  ...
  Future<void> _onLogoutRequested(LogoutRequested event,Emitter<LoginState> emit) async {
    ...
    if (kDebugMode) {
      stopwatch = Stopwatch()..start();
      print('🚪 Procesando logout en BLoC...');
    }
    ...
    try {
      ...
      if (kDebugMode) {
        stopwatch?.stop();
        print('✅ Logout BLoC completado en ${stopwatch?.elapsedMilliseconds}ms - Success: $logoutSuccess');
      }
      ...
      if (logoutSuccess) {
        ...
        if (kDebugMode) print('🔄 Estado del BLoC reseteado');
      } else {
        // Error en logout
        emit(state.copyWith(
      ...
      if (kDebugMode) {
        stopwatch?.stop();
        print('💥 Error en logout BLoC (${stopwatch?.elapsedMilliseconds}ms): $e');
      }
